...
/* Skapar ett fristående avtal som inte är kopplat till en bokning.
   Används för samtycke, brudpar, företagslicens och övriga avtal.
   detaljer.typ styr hur den publika signeringssidan renderas. */
export async function skapaFristaendeAvtal(formData: FormData) {
  const supabase = await createClient();
  const userRes = await supabase.auth.getUser();
  const user = userRes.data.user;
  if (!user) return;

  const typ = String(formData.get('typ') || 'ovrigt').trim() || 'ovrigt';
  const typ_label = String(formData.get('typ_label') || 'Avtal').trim() || 'Avtal';
  const kund_namn = String(formData.get('kund_namn') || '').trim();
  const kund_email = String(formData.get('kund_email') || '').trim();
  const personligt_meddelande = String(formData.get('personligt_meddelande') || '').trim() || null;
